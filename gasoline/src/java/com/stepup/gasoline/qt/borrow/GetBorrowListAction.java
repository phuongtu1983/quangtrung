 ...
 * @author phuongtu
 */
public class GetBorrowListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList borrowList = employeeDAO.searchBorrow(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (borrowList != null) {
                int length = borrowList.size();
                for (int i = 0; i < length; i++) {
                    BorrowFormBean bean = (BorrowFormBean) borrowList.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getBorrow(").append(bean.getId()).append(")^_self</cell>");
