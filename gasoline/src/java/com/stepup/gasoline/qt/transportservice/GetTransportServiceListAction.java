 ...
 * @author phuongtu
 */
public class GetTransportServiceListAction extends BaseAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList list = vehicleDAO.searchTransportService(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (list != null) {
                int length = list.size();
                for (int i = 0; i < length; i++) {
                    TransportServiceFormBean bean = (TransportServiceFormBean) list.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getTransportService(").append(bean.getId()).append(",\"loadTransportServicePanel\")^_self</cell>");
