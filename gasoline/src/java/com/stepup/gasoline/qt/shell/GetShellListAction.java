 ...
 * @author phuongtu
 */
public class GetShellListAction extends BaseAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList list = shellDAO.getShells(NumberUtil.parseInt(request.getParameter("status"), 0));
            if (list != null) {
                int length = list.size();
                for (int i = 0; i < length; i++) {
                    ShellFormBean bean = (ShellFormBean) list.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getShell(").append(bean.getId()).append(",\"loadShellList\")^_self</cell>");
