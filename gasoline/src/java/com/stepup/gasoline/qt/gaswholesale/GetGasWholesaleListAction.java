 ...
 * @author phuongtu
 */
public class GetGasWholesaleListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList importList = gasDAO.searchGasWholesale(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (importList != null) {
                int length = importList.size();
                for (int i = 0; i < length; i++) {
                    GasWholesaleFormBean bean = (GasWholesaleFormBean) importList.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getGasWholesale(").append(bean.getId()).append(")^_self</cell>");
