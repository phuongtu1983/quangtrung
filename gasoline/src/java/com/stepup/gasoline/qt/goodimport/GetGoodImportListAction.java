 ...
 * @author phuongtu
 */
public class GetGoodImportListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList importList = goodDAO.searchGoodImport(request.getParameter("fromDate"), request.getParameter("toDate"));
            if (importList != null) {
                int length = importList.size();
                for (int i = 0; i < length; i++) {
                    GoodImportFormBean bean = (GoodImportFormBean) importList.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getGoodImport(").append(bean.getId()).append(")^_self</cell>");
