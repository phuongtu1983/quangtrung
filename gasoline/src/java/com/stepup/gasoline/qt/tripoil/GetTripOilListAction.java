 ...
 * @author phuongtu
 */
public class GetTripOilListAction extends BaseAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList list = vehicleDAO.searchTripOil(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (list != null) {
                int length = list.size();
                for (int i = 0; i < length; i++) {
                    TripOilFormBean bean = (TripOilFormBean) list.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getTripOil(").append(bean.getId()).append(",\"loadTripOilPanel\")^_self</cell>");
