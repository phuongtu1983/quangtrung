 ...
 * @author phuongtu
 */
public class GetSolarImportListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList importList = goodDAO.searchSolarImport(request.getParameter("fromDate"), request.getParameter("toDate"));
            if (importList != null) {
                int length = importList.size();
                for (int i = 0; i < length; i++) {
                    SolarImportFormBean bean = (SolarImportFormBean) importList.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getSolarImport(").append(bean.getId()).append(")^_self</cell>");
