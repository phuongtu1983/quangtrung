 ...
 * @author phuongtu
 */
public class GetSaleGoodListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList importList = goodDAO.searchSaleGood(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (importList != null) {
                int length = importList.size();
                for (int i = 0; i < length; i++) {
                    SaleGoodFormBean bean = (SaleGoodFormBean) importList.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getSaleGood(").append(bean.getId()).append(")^_self</cell>");
