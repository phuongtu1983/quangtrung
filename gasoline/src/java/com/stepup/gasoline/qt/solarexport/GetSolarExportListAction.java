 ...
 * @author phuongtu
 */
public class GetSolarExportListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList exportList = goodDAO.searchSolarExport(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (exportList != null) {
                int length = exportList.size();
                for (int i = 0; i < length; i++) {
                    SolarExportFormBean bean = (SolarExportFormBean) exportList.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getSolarExport(").append(bean.getId()).append(")^_self</cell>");
