 ...
 * @author phuongtu
 */
public class GetDebtAdjustmentListAction extends BaseAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList list = paymentDAO.searchDebtAdjustment(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (list != null) {
                int length = list.size();
                for (int i = 0; i < length; i++) {
                    DebtAdjustmentFormBean bean = (DebtAdjustmentFormBean) list.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getDebtAdjustment(").append(bean.getId()).append(",\"loadDebtAdjustmentPanel\")^_self</cell>");
