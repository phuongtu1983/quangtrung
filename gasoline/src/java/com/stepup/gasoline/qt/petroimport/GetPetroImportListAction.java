 ...
 * @author phuongtu
 */
public class GetPetroImportListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList importList = gasDAO.searchPetroImport(request.getParameter("fromDate"), request.getParameter("toDate"));
            if (importList != null) {
                int length = importList.size();
                for (int i = 0; i < length; i++) {
                    PetroImportFormBean bean = (PetroImportFormBean) importList.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getPetroImport(").append(bean.getId()).append(")^_self</cell>");
                    ...
                    buff.append("<cell>").append(StringUtil.encodeString(bean.getStoreName())).append("</cell>");
