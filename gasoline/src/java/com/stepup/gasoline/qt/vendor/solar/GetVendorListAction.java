 ...
 * @author phuongtu
 */
public class GetVendorListAction extends BaseAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList list = vendorDAO.getVendors(QTUtil.getOrganizationManageds(request.getSession()), VendorBean.IS_SOLAR + "");
            if (list != null) {
                int length = list.size();
                for (int i = 0; i < length; i++) {
                    VendorFormBean bean = (VendorFormBean) list.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getVendorSolar(").append(bean.getId()).append(",\"loadVendorList\")^_self</cell>");
