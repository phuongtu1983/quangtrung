 ...
 * @author phuongtu
 */
public class GetOilExportListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList exportList = goodDAO.searchOilExport(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (exportList != null) {
                int length = exportList.size();
                for (int i = 0; i < length; i++) {
                    OilExportFormBean bean = (OilExportFormBean) exportList.get(i);
