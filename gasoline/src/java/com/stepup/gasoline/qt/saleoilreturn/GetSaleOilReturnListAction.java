...
import com.stepup.gasoline.qt.saleoil.*;
 ...
 * @author phuongtu
 */
public class GetSaleOilReturnListAction extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList importList = goodDAO.searchSaleOilReturn(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (importList != null) {
                int length = importList.size();
                for (int i = 0; i < length; i++) {
                    SaleOilFormBean bean = (SaleOilFormBean) importList.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getSaleOilReturn(").append(bean.getId()).append(")^_self</cell>");
