 ...
 * @author phuongtu
 */
public class AddSolarAction extends SpineAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        SolarFormBean formBean = (SolarFormBean) form;
        GoodDAO goodDAO = new GoodDAO();
        SolarBean bean = null;
        boolean bNew = false;
        boolean isExist = false;
        try {
            bean = goodDAO.getSolarByName(formBean.getName());
        } catch (Exception ex) {
        }
        ...
        if (isExist) {
            ...
            errors.add("solarExisted", new ActionMessage("solar.save.error"));
        ...
        try {
            if (bNew) {
                goodDAO.insertSolar(bean);
            } else {
                if (isUpdate) {
                    goodDAO.updateSolar(bean);
                }
            }
        } catch (Exception ex) {
            ex.printStackTrace();
