 ...
 * @author phuongtu
 */
public class GetDebtWholesaleListAction extends BaseAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList list = paymentDAO.searchDebtWholesale(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (list != null) {
                int length = list.size();
                for (int i = 0; i < length; i++) {
                    DebtWholesaleFormBean bean = (DebtWholesaleFormBean) list.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getDebtWholesale(").append(bean.getId()).append(",\"loadDebtWholesalePanel\")^_self</cell>");
