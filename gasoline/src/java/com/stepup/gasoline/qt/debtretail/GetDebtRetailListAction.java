 ...
 * @author phuongtu
 */
public class GetDebtRetailListAction extends BaseAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList list = paymentDAO.searchDebtRetail(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (list != null) {
                int length = list.size();
                for (int i = 0; i < length; i++) {
                    DebtRetailFormBean bean = (DebtRetailFormBean) list.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getDebtRetail(").append(bean.getId()).append(",\"loadDebtRetailPanel\")^_self</cell>");
