 ...
 * @author phuongtu
 */
public class GetDebtVendorListAction extends BaseAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList list = paymentDAO.searchDebtVendor(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (list != null) {
                int length = list.size();
                for (int i = 0; i < length; i++) {
                    DebtVendorFormBean bean = (DebtVendorFormBean) list.get(i);
