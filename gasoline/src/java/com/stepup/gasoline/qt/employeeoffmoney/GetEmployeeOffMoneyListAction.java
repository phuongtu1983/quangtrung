 ...
 * @author phuongtu
 */
public class GetEmployeeOffMoneyListAction extends BaseAction {
    ...
    public boolean doAction(ActionMapping mapping, ActionForm form,
        ...
        try {
            ...
            ArrayList list = employeeDAO.searchEmployeeOffMoney(request.getParameter("fromDate"), request.getParameter("toDate"), QTUtil.getOrganizationManageds(request.getSession()));
            if (list != null) {
                int length = list.size();
                for (int i = 0; i < length; i++) {
                    EmployeeOffMoneyFormBean bean = (EmployeeOffMoneyFormBean) list.get(i);
                    buff.append("<row id=\"").append(bean.getId()).append("\">");
                    buff.append("<cell>").append(bean.getCode()).append("^javascript:getEmployeeOffMoney(").append(bean.getId()).append(",\"loadEmployeeOffMoneyPanel\")^_self</cell>");
