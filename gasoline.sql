...
BEGIN
	...
	IF _kind=1 THEN -- export_customer_opening_stock
		...
		(
			...
			UNION ALL
			SELECT i.customer_id, 0 AS quantity_12, 0 AS quantity_45, COALESCE(i.debt - i.paid_old_debt_amount,0) AS amount
			...
			WHERE DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date and gas_wholesale_id=0
			UNION ALL
	...
	ELSEIF _kind=3 THEN -- set value
		...
		(
			...
			FROM customer_in_stock
			WHERE customer_id=_customer_id and DATEDIFF(`day`, _from_date) = 0 -- AND DATEDIFF(`day`, _to_date) <= 0 
			UNION ALL
			...
			UNION ALL
			SELECT 0 AS quantity_12, 0 AS quantity_45, COALESCE(i.debt - i.paid_old_debt_amount,0) AS amount, 0 AS transport_amount
			FROM gas_wholesale AS i
			...
			FROM wholesale_debt AS i
			WHERE DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date AND i.customer_id=_customer_id AND gas_wholesale_id=0
			UNION ALL
...
BEGIN
	...
	IF _is_list=1 THEN
		...
		FROM organization AS o, store AS so
		...
		(
			...
			WHERE i_det.petro_sale_id=i.id AND DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date 
				AND i.store_id=s.id
		) AS tbl ON tbl.organization_id=so.organization_id AND tbl.petro_id=a.id AND tbl.store_id=so.id
		WHERE a.STATUS=1 AND o.STATUS=1 AND so.STATUS=1 AND so.is_petro=1 and o.id=so.organization_id
		GROUP BY o.id, so.id, a.id
		ORDER BY o.NAME, so.NAME, a.NAME
		;
	END IF;
...
*/
			...
			FROM shell_vendor AS sv, gas_wholesale_detail AS i_det, gas_wholesale AS i
			WHERE DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date 
				AND i_det.gas_wholesale_id=i.id and sv.id=i_det.shell_id
			UNION ALL
			SELECT sv.shell_id, sv.organization_id, -COALESCE(f_det.quantity,0) AS in_stock
			FROM shell_vendor AS sv, gas_export_wholesale_detail AS f_det, gas_export_wholesale AS f
			WHERE DATE(f.created_date) >= _from_date AND DATE(f.created_date) < _to_date 
				AND f_det.gas_export_wholesale_id=f.id AND sv.id=f_det.shell_id
			UNION ALL
			SELECT sv.shell_id, sv.organization_id, -COALESCE(i_det.quantity,0) AS in_stock
			FROM shell_vendor AS sv, inner_sale_detail AS i_det, inner_sale AS i
			WHERE DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date 
				AND i_det.inner_sale_id=i.id AND sv.id=i_det.shell_id
...
BEGIN
	...
	IF _kind=1 THEN -- export_customer_opening_stock
		...
		(
			...
			UNION ALL
			SELECT i.vendor_id, eo.organization_id, -COALESCE(i.amount,0) AS amount, 0 AS quantity_12, 0 AS quantity_45, -COALESCE(i.amount,0) AS transport_debt
			FROM debt_adjustment AS i, employee AS eo
			WHERE DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date
				AND i.created_employee_id=eo.id AND i.vendor_id<>0 AND i.kind=3
			UNION ALL
			SELECT i.vendor_id, eo.organization_id, COALESCE(i.debt,0) AS amount, 0 AS quantity_12, 0 AS quantity_45, 0 AS transport_debt
...
*/		
			SELECT idet.vendor_id, eo.organization_id, -SUM(idet.amount*(100-v.commision_on_import))/100 AS amount, 0 AS quantity_12, 0 AS quantity_45, 0 AS transport_debt
			FROM oil_sale_detail AS idet, oil_sale AS i, employee AS eo, vendor AS v, oil_return_store AS s
			WHERE DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date
				AND i.created_employee_id=eo.id AND i.id=idet.oil_sale_id
				AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%')
				AND idet.vendor_id=v.id AND v.commision_on_import>0 AND idet.store_id<>s.store_id AND _organization_ids LIKE CONCAT('%,',s.organization_id,',%') 
		) AS tbl ON tbl.organization_id=vo.organization_id AND tbl.vendor_id=vo.vendor_id
	...
	ELSEIF _kind=3 THEN -- set value
		...
		(
			...
			FROM vendor_in_stock
			WHERE DATEDIFF(`day`, _from_date) = 0 -- AND DATEDIFF(`day`, _to_date) <= 0 
				AND vendor_id=_vendor_id AND _organization_ids LIKE CONCAT('%,',organization_id,',%')
			UNION ALL
			SELECT i.vendor_id, eo.organization_id, COALESCE(i.debt,0) AS amount, 0 AS quantity_12, 0 AS quantity_45, 0 AS transport_debt
			FROM lpg_import AS i, employee AS eo
			WHERE DATE(i.import_date) >= _from_date AND DATE(i.import_date) < _to_date
			...
			FROM debt_adjustment AS i, employee AS eo
			WHERE DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date
				AND i.created_employee_id=eo.id AND i.vendor_id<>0 AND i.kind=3  AND i.vendor_id=_vendor_id
				 AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%')
			UNION ALL
			SELECT i.vendor_id, eo.organization_id, COALESCE(i.debt,0) AS amount, 0 AS quantity_12, 0 AS quantity_45, 0 AS transport_debt
			FROM good_import AS i, employee AS eo
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `insertSalePetro`(IN _code VARCHAR(20), IN _created_date VARCHAR(20), in _customer_id int, in _store_id int, IN _total DOUBLE
	, IN _paid DOUBLE, IN _debt DOUBLE, IN _discount DOUBLE, IN _total_pay DOUBLE, IN _account_id INT, IN _note TEXT, IN _created_employee_id INT, IN _old_paid_debt DOUBLE, OUT _id INT)
BEGIN
	INSERT INTO petro_sale (CODE, created_date, customer_id, store_id, total, paid, debt, discount, total_pay, account_id, note, created_employee_id, paid_old_debt_amount)
	VALUES (_code, STR_TO_DATE(_created_date,'%d/%m/%Y'), _customer_id, _store_id, _total, _paid, _debt, _discount, _total_pay, _account_id, _note, _created_employee_id, _old_paid_debt);
	SELECT LAST_INSERT_ID() INTO _id;
    END */$$
DELIMITER ;
...
BEGIN
	...
	-- thu
		...
		SELECT o.created_date, IF(a.is_cash=0,o.paid,0) AS account_income_amount, 0 AS account_outcome_amount, IF(a.is_cash=0,o.note,'') AS account_note
			, IF(a.is_cash=1,o.paid,0) AS cash_income_amount, 0 AS cash_outcome_amount, IF(a.is_cash=1,o.note,'') AS cash_note
		FROM oil_sale AS o, account AS a
		WHERE DATE(o.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(o.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			AND o.account_id=a.id AND _organization_ids LIKE CONCAT('%,',a.organization_id,',%')
		union all
		SELECT o.created_date, IF(a.is_cash=0,o.paid,0) AS account_income_amount, 0 AS account_outcome_amount, IF(a.is_cash=0,o.note,'') AS account_note
			, IF(a.is_cash=1,o.paid,0) AS cash_income_amount, 0 AS cash_outcome_amount, IF(a.is_cash=1,o.note,'') AS cash_note
		FROM solar_sale AS o, account AS a
		WHERE DATE(o.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(o.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			AND o.account_id=a.id AND _organization_ids LIKE CONCAT('%,',a.organization_id,',%')
	-- chi
		UNION ALL
		...
		SELECT o.created_date, 0 AS account_income_amount, IF(a.is_cash=0,o.paid,0) AS account_outcome_amount, IF(a.is_cash=0,o.note,'') AS account_note
			, 0 AS cash_income_amount, IF(a.is_cash=1,o.paid,0) AS cash_outcome_amount, IF(a.is_cash=1,o.note,'') AS cash_note
		FROM oil_import AS o, account AS a
		WHERE DATE(o.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(o.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			AND o.account_id=a.id AND _organization_ids LIKE CONCAT('%,',a.organization_id,',%')
		UNION ALL
		SELECT o.created_date, 0 AS account_income_amount, IF(a.is_cash=0,o.paid,0) AS account_outcome_amount, IF(a.is_cash=0,o.note,'') AS account_note
			, 0 AS cash_income_amount, IF(a.is_cash=1,o.paid,0) AS cash_outcome_amount, IF(a.is_cash=1,o.note,'') AS cash_note
		FROM solar_import AS o, account AS a
		WHERE DATE(o.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(o.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			AND o.account_id=a.id AND _organization_ids LIKE CONCAT('%,',a.organization_id,',%')
	) AS tbl
	GROUP BY tbl.created_date
	having account_income_amount<>0 or account_outcome_amount<>0 or cash_income_amount<>0 or cash_outcome_amount<>0
	ORDER BY tbl.created_date;
END */$$
DELIMITER ;

/* Procedure structure for procedure `report_cash_book_module` */

/*!50003 DROP PROCEDURE IF EXISTS  `report_cash_book_module` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `report_cash_book_module`(IN _start_date VARCHAR(20), IN _end_date VARCHAR(20), IN _organization_ids TEXT, in _module varchar(20))
BEGIN	
	if _module='moduleGas' then
		SELECT tbl.created_date, SUM(tbl.account_income_amount) AS account_income_amount, SUM(tbl.account_outcome_amount) AS account_outcome_amount, '' AS account_note
			, SUM(tbl.cash_income_amount) AS cash_income_amount, SUM(tbl.cash_outcome_amount) AS cash_outcome_amount, '' AS cash_note
		FROM 
		(
	...
	elseif _module='moduleOil' then
		...
		GROUP BY tbl.created_date ORDER BY tbl.created_date;
	elseif _module='moduleSolar' then
		SELECT tbl.created_date, SUM(tbl.account_income_amount) AS account_income_amount, SUM(tbl.account_outcome_amount) AS account_outcome_amount, '' AS account_note
			, SUM(tbl.cash_income_amount) AS cash_income_amount, SUM(tbl.cash_outcome_amount) AS cash_outcome_amount, '' AS cash_note
		FROM 
		(
		...
		GROUP BY tbl.created_date ORDER BY tbl.created_date;
	ELSEIF _module='modulePetro' THEN
		SELECT tbl.created_date, SUM(tbl.account_income_amount) AS account_income_amount, SUM(tbl.account_outcome_amount) AS account_outcome_amount, '' AS account_note
			, SUM(tbl.cash_income_amount) AS cash_income_amount, SUM(tbl.cash_outcome_amount) AS cash_outcome_amount, '' AS cash_note
		FROM 
		(
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `report_compare_lpg_vendor_customer`(IN _start_date VARCHAR(20), IN _end_date VARCHAR(20), IN _organization_ids TEXT
	, IN _vendor_customer_id INT, OUT _opening_stock DOUBLE, out _vendor_name text)
BEGIN
	...
	select `name` into _vendor_name from vendor where id=_vendor_id;
	...
	SELECT * FROM(
		...
		SELECT ls.created_date AS created_date, 'VAN' AS content_code, 'Vận chuyển' AS content, 0 AS in_quantity, 0 AS out_quantity, ls.in_quantity AS transport_quantity
			, ls.price, ls.rate, -(ls.vendor_amount + ls.customer_amount) AS amount, -ls.paid as paid, -ls.debt as debt, ls.note, 0 AS link_lpg_quantity
		...
		WHERE DATE(ls.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(ls.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			AND ls.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%') 
			AND (ls.vendor_id=_vendor_id OR ls.customer_id=_customer_id) and ls.kind=1 -- dieu chinh tien hang
		UNION ALL
		SELECT ls.created_date AS created_date, 'DIEU' AS content_code, 'Điều chỉnh' AS content, 0 AS in_quantity, 0 AS out_quantity, 0 AS transport_quantity
			, 0 AS price, 0 AS rate, ls.amount AS amount, 0 AS paid, 0 AS debt, ls.note, 0 AS link_lpg_quantity
		FROM debt_adjustment AS ls, employee AS eo
		WHERE DATE(ls.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(ls.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			AND ls.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%') 
			AND (ls.vendor_id=_vendor_id OR ls.customer_id=_customer_id) and ls.kind=2 -- dieu chinh tien van chuyen
		UNION ALL
...
BEGIN
	...
	IF _employee_id=0 THEN
		...
		UNION ALL
		SELECT _session_id, -lv.id as vehicle_out_id, lv.created_date, 0 AS quantity_12, 0 AS quantity_45, lv.quantity AS quantity_lovo
...
BEGIN
	...
	GROUP BY tbl.vehicle_out_id
	UNION ALL
	SELECT _session_id, -lv.id as vehicle_out_id, lv.created_date, 'lovo', 0 AS quantity_12, 0 AS quantity_45, lv.quantity AS quantity_lovo
		, 0 AS accessory_kind_id, 0 AS quantity_accessory, 0 AS commission_accessory
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `report_oil_compare`(IN _start_date VARCHAR(20), IN _end_date VARCHAR(20), IN _organization_ids TEXT, IN _customer_id INT
	, OUT _amount_debt DOUBLE, OUT _customer_paid DOUBLE, out _customer_commission float)
BEGIN
	CALL get_in_stock_customer(_start_date, 3, _customer_id, _amount_debt, @a, @a, @a);
	
	select commission_percentage into _customer_commission from customer where id=_customer_id;
	
	SELECT COALESCE(SUM(paid),0) INTO _customer_paid FROM wholesale_debt WHERE customer_id=_customer_id
		AND DATE(created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y');
	...
	FROM oil_sale as os
	left join oil_sale_detail as osdet on os.id=osdet.oil_sale_id 
	left join oil AS o on osdet.oil_id=o.id
	, employee AS eo
	WHERE DATE(os.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(os.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
		AND os.customer_id=_customer_id AND os.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%')
	UNION ALL
	SELECT a.created_date, '' AS oil_code, '' AS oil_name, 0 AS quantity, 0 AS price, 0 AS amount, a.amount AS paid
		, 0 AS commission
	FROM debt_adjustment AS a, employee AS eo
	WHERE DATE(a.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(a.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
		AND a.customer_id=_customer_id AND a.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%')
	;
    END */$$
DELIMITER ;
...
/* Procedure structure for procedure `report_oil_sale_customer` */

/*!50003 DROP PROCEDURE IF EXISTS  `report_oil_sale_customer` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `report_oil_sale_customer`(IN _start_date VARCHAR(20), IN _end_date VARCHAR(20), IN _organization_ids TEXT)
BEGIN
	DECLARE _from_date, _to_date DATE;
	...
	SELECT c.`code` AS customer_code, c.`name` AS customer_name, COALESCE(cin.amount,0) AS opening_stock, COALESCE(tbl_oil_sale.debt,0) AS debt
		, COALESCE(tbl_oil_sale.total_pay,0) AS amount
	FROM customer AS c
	LEFT JOIN customer_in_stock AS cin ON c.id=cin.customer_id AND DATEDIFF(cin.`day`, _from_date) = 0
	LEFT JOIN (
		SELECT customer_id, COALESCE(SUM(total_pay),0) AS total_pay, COALESCE(SUM(debt),0) AS debt
		FROM oil_sale
		WHERE DATE(created_date) >= _from_date AND DATE(created_date) < _to_date
		GROUP BY customer_id
	) AS tbl_oil_sale ON tbl_oil_sale.customer_id=c.id
	WHERE c.STATUS=1 AND c.is_oil=1 AND _organization_ids LIKE CONCAT('%,',c.organization_id,',%') 
	HAVING opening_stock<>0 OR debt<>0 OR amount<>0
	;
    END */$$
DELIMITER ;
	...
	IF _oil_id=0 THEN
		...
		(
			...
			(
				...
				FROM oil_in_stock
				WHERE DATEDIFF(`day`, _from_date) = 0 AND _organization_ids LIKE CONCAT('%,',organization_id,',%') -- AND DATEDIFF(`day`, _to_date) <= 0 
				union all
...
/*			SELECT idet.vendor_id, eo.organization_id, -COALESCE(idet.quantity*idet.price_before_commission,0) AS debt
			...
			WHERE DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date
				AND i.created_employee_id=eo.id AND i.id=idet.oil_sale_id
					 AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%')
*/
			SELECT idet.vendor_id, eo.organization_id, sum(idet.amount*(100-v.commision_on_import))/100 AS debt
			FROM oil_sale_detail AS idet, oil_sale AS i, employee AS eo, vendor as v, oil_return_store as s
			WHERE DATE(i.created_date) >= _from_date AND DATE(i.created_date) < _to_date
				AND i.created_employee_id=eo.id AND i.id=idet.oil_sale_id
				AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%')
...
/*		SELECT oi.vendor_id, eo.organization_id, SUM(oi.total) AS amount, SUM(oi.paid) AS paid
		...
		UNION ALL */
		SELECT idet.vendor_id, eo.organization_id, sum(idet.amount*(100-v.commision_on_import))/100 AS amount, 0 as paid
		FROM oil_sale_detail AS idet, oil_sale AS i, employee AS eo, vendor as v, oil_return_store as s
		WHERE DATE(i.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(i.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y') 
			AND i.created_employee_id=eo.id AND i.id=idet.oil_sale_id
			AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%')
...
BEGIN
	...
	select o.code as oil_code, o.name as oil_name, coalesce(opening_stock,0) as opening_stock
		, SUM(import_tbl.quantity) AS import_quantity, sum(export_tbl.quantity) AS export_quantity , AVG(export_tbl.export_price) AS export_price
	...
	LEFT JOIN
	(
		SELECT idet.oil_id, SUM(idet.base_quantity) AS quantity, avg(idet.price) as export_price
		...
		union all
		SELECT idet.oil_id, SUM(idet.base_quantity) AS quantity, 0 as export_price
		...
		GROUP BY idet.oil_id
		union all
		SELECT i_det.oil_id, -SUM(i_det.base_quantity) AS quantity, avg(i_det.price) as export_price
		FROM oil_sale_return_detail AS i_det, oil_sale_return AS i, store AS s
		WHERE DATE(i.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(i.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
...
BEGIN
	SELECT p.id, p.created_date, c.CODE AS customerCode, c.NAME AS customerName, s.NAME AS storeName, pe.code AS petroCode, pdet.quantity, pdet.price, pdet.amount, p.paid, a.number AS account
...
/* Procedure structure for procedure `report_petro_sale_customer` */

/*!50003 DROP PROCEDURE IF EXISTS  `report_petro_sale_customer` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `report_petro_sale_customer`(IN _start_date VARCHAR(20), IN _end_date VARCHAR(20), IN _organization_ids TEXT)
BEGIN
	DECLARE _from_date, _to_date DATE;
	...
	select c.`code` as customer_code, c.`name` as customer_name, COALESCE(cin.amount,0) as opening_stock, COALESCE(tbl_petro_sale.debt,0) as debt
		, COALESCE(tbl_petro_sale.total_pay,0) as amount
	from customer as c
	left join customer_in_stock as cin on c.id=cin.customer_id and DATEDIFF(cin.`day`, _from_date) = 0
	left join (
		select customer_id, coalesce(sum(total_pay),0) as total_pay, coalesce(sum(debt),0) as debt
		...
		where DATE(created_date) >= _from_date AND DATE(created_date) < _to_date
		group by customer_id
	) as tbl_petro_sale on tbl_petro_sale.customer_id=c.id
	where c.status=1 and c.is_petro=1 AND _organization_ids LIKE CONCAT('%,',c.organization_id,',%') 
	having opening_stock<>0 or debt<>0 or amount<>0
	;
    END */$$
DELIMITER ;
...
BEGIN
	...
	if _petro_id=0 then
		...
		select COALESCE(GROUP_CONCAT(distinct petro_id),'') into _petro_ids from temp_petro_stock where session_id=_session_id;
...
BEGIN
	...
	SELECT c.CODE AS customer_code, c.NAME AS customer_name, SUM(COALESCE(tbl.quantity_12,0)) AS quantity_12, SUM(COALESCE(tbl.quantity_45,0)) AS quantity_45
		...
		, SUM(COALESCE(tbl.total,0)) + SUM(COALESCE(tbl_lpg.amount - tbl_lpg.paid,0)) AS amount, SUM(COALESCE(tbl.debt,0)) + SUM(COALESCE(tbl_lpg.debt,0)) AS debt
	...
	(
		SELECT w.customer_id, SUM(IF(shell_12.id IS NOT NULL, wdet.quantity,0)) AS quantity_12, SUM(IF(shell_45.id IS NOT NULL, wdet.quantity,0)) AS quantity_45
			, SUM(COALESCE(wdet.amount,0)) AS total, tbl_gas_wholesale.debt AS debt
		FROM gas_wholesale AS w
		LEFT JOIN gas_wholesale_detail AS wdet ON w.id=wdet.gas_wholesale_id
		LEFT JOIN shell_vendor AS sv ON wdet.shell_id=sv.id AND _vendor_ids LIKE CONCAT('%,',sv.vendor_id,',%')
		LEFT JOIN (SELECT s.id FROM shell AS s, shell_kind AS sk WHERE s.kind_id=sk.id AND sk.weight=12) AS shell_12 ON sv.shell_id=shell_12.id
		LEFT JOIN (SELECT s.id FROM shell AS s, shell_kind AS sk WHERE s.kind_id=sk.id AND sk.weight=45) AS shell_45 ON sv.shell_id=shell_45.id
		...
		LEFT JOIN (
			SELECT customer_id, SUM(debt - paid_old_debt_amount) AS debt 
			FROM gas_wholesale 
			WHERE DATE(created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			GROUP BY customer_id
		) AS tbl_gas_wholesale ON tbl_gas_wholesale.customer_id=w.customer_id
		WHERE DATE(w.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(w.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
	...
	LEFT JOIN
	(
		SELECT w.customer_id, SUM(IF(shell_12.id IS NOT NULL, return_shell_12.quantity,0)) AS shell_return_12
		FROM gas_wholesale AS w
		...
		WHERE DATE(w.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(w.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
		GROUP BY w.customer_id
	) AS tbl_return12 ON tbl_return12.customer_id=tbl.customer_id
	LEFT JOIN
	(
		SELECT w.customer_id, SUM(IF(shell_45.id IS NOT NULL, return_shell_45.quantity,0)) AS shell_return_45
		FROM gas_wholesale AS w
		...
		WHERE DATE(w.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(w.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
		GROUP BY w.customer_id
	) AS tbl_return45 ON tbl_return45.customer_id=tbl.customer_id
	LEFT JOIN
	(
		SELECT tbl.customer_id, SUM(tbl.amount) AS amount, SUM(tbl.paid) AS paid, SUM(tbl.debt) AS debt
		FROM (
			SELECT ls.customer_id, ls.amount, ls.paid, ls.debt
			FROM lpg_sale AS ls, employee AS eo
			WHERE DATE(ls.sale_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(ls.sale_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
				AND ls.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%')
			UNION ALL
			SELECT wd.customer_id, 0 AS amount, wd.paid, -wd.paid AS debt
			FROM wholesale_debt AS wd, employee AS eo
			WHERE DATE(wd.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(wd.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
				AND wd.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%') AND wd.gas_wholesale_id=0
		) AS tbl GROUP BY tbl.customer_id
	) AS tbl_lpg ON tbl_lpg.customer_id=c.id
	WHERE  _organization_ids LIKE CONCAT('%,',c.organization_id,',%') AND c.STATUS=1 AND c.is_gas=1
	GROUP BY c.id
	ORDER BY c.NAME;
    END */$$
DELIMITER ;
...
BEGIN
	...
	SELECT os.created_date, o.CODE AS solar_code, o.NAME AS solar_name, osdet.quantity, osdet.price, osdet.amount, os.paid
		, (osdet.amount * esd.commission / 100) AS commission
	FROM solar_sale AS os
	left join solar_sale_detail AS osdet on os.id=osdet.solar_sale_id
	left join solar AS o on osdet.solar_id=o.id
	left join employee_solar_commission_detail as esd on esd.commission_from<=o.weight AND esd.commission_to>o.weight
	, employee AS eo
	WHERE DATE(os.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(os.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
		AND os.customer_id=_customer_id AND os.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%')
	UNION ALL
...
/* Procedure structure for procedure `report_solar_sale_customer` */

/*!50003 DROP PROCEDURE IF EXISTS  `report_solar_sale_customer` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `report_solar_sale_customer`(IN _start_date VARCHAR(20), IN _end_date VARCHAR(20), IN _organization_ids TEXT)
BEGIN
	DECLARE _from_date, _to_date DATE;
	...
	SELECT c.`code` AS customer_code, c.`name` AS customer_name, COALESCE(cin.amount,0) AS opening_stock, COALESCE(tbl_solar_sale.debt,0) AS debt
		, COALESCE(tbl_solar_sale.total_pay,0) AS amount
	FROM customer AS c
	LEFT JOIN customer_in_stock AS cin ON c.id=cin.customer_id AND DATEDIFF(cin.`day`, _from_date) = 0
	LEFT JOIN (
		SELECT customer_id, COALESCE(SUM(total_pay),0) AS total_pay, COALESCE(SUM(debt),0) AS debt
		...
		WHERE DATE(created_date) >= _from_date AND DATE(created_date) < _to_date
		GROUP BY customer_id
	) AS tbl_solar_sale ON tbl_solar_sale.customer_id=c.id
	WHERE c.STATUS=1 AND c.is_solar=1 AND _organization_ids LIKE CONCAT('%,',c.organization_id,',%') 
	HAVING opening_stock<>0 OR debt<>0 OR amount<>0
	;
    END */$$
DELIMITER ;
...
BEGIN
	...
	(
		...
		(
			...
			FROM(
				SELECT ws.vehicle_out_id, SUM(wholesale_detail_tbl.total) AS total, SUM(ws.paid + ws.paid_old_debt_amount) AS paid 
				FROM gas_wholesale AS ws left join
				(
					...
					WHERE w.id=det.gas_wholesale_id AND w.vehicle_out_id<>0
						AND DATE(w.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(w.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
					GROUP BY w.id
				) AS wholesale_detail_tbl on wholesale_detail_tbl.id=ws.id
				WHERE DATE(ws.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(ws.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y') 
					AND ws.vehicle_out_id<>0 
				...
				WHERE DATE(wd.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(wd.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y') 
					AND wd.gas_wholesale_id=gw.id AND gw.vehicle_out_id<>0 AND wd.gas_wholesale_id is null
...
BEGIN
	...
	SELECT v.CODE AS vendor_code, v.NAME AS vendor_name, tbl_vendor_stock.amount AS opening_stock
		, COALESCE(SUM(tbl_lpg_import.amount),0) + coalesce(SUM(tbl_debt_adjustment.amount),0) AS amount
		, COALESCE(SUM(tbl_vendor_paid.paid),0) + coalesce(SUM(tbl_debt_adjustment.paid),0) AS paid
	FROM vendor AS v, organization AS o, vendor_organization AS vo
	LEFT JOIN (
		SELECT vo.organization_id, vo.vendor_id, SUM(COALESCE(stock.debt,0)) AS amount
		FROM organization AS o, vendor AS v, vendor_organization AS vo
	...
	LEFT JOIN(
		SELECT i.vendor_id, eo.organization_id, sum(i.amount) as amount
		FROM lpg_import AS i, employee AS eo
		WHERE DATE(i.import_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(i.import_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			AND i.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%') 
		group by i.vendor_id
	) AS tbl_lpg_import ON vo.vendor_id=tbl_lpg_import.vendor_id AND tbl_lpg_import.organization_id=vo.organization_id
	LEFT JOIN(
		SELECT i.vendor_id, eo.organization_id, sum(i.paid) as paid
		FROM vendor_debt AS i, employee AS eo
		WHERE DATE(i.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(i.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			AND i.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%') 
		group by i.vendor_id
	) AS tbl_vendor_paid ON vo.vendor_id=tbl_vendor_paid.vendor_id AND tbl_vendor_paid.organization_id=vo.organization_id
	LEFT JOIN(
		SELECT i.vendor_id, eo.organization_id, SUM(if(i.kind=1,i.amount,0)) AS amount, SUM(if(i.kind=3,i.amount,0)) AS paid
		FROM debt_adjustment AS i, employee AS eo
		WHERE DATE(i.created_date) >= STR_TO_DATE(_start_date,'%d/%m/%Y') AND DATE(i.created_date) <= STR_TO_DATE(_end_date,'%d/%m/%Y')
			AND i.created_employee_id=eo.id AND _organization_ids LIKE CONCAT('%,',eo.organization_id,',%') 
			and (i.kind=3 or i.kind=1)
		GROUP BY i.vendor_id
	) AS tbl_debt_adjustment ON vo.vendor_id=tbl_debt_adjustment.vendor_id AND tbl_debt_adjustment.organization_id=vo.organization_id
	WHERE vo.vendor_id=v.id AND vo.organization_id=o.id AND v.STATUS=1 AND o.STATUS=1 AND v.is_gas=1
	GROUP BY v.id	
	HAVING opening_stock<> 0 OR amount<>0 OR paid<>0
	;
    END */$$
DELIMITER ;
...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `updateSalePetro`(IN _id INT, IN _created_date VARCHAR(20), in _customer_id int, in _store_id int, IN _total DOUBLE
	, IN _paid DOUBLE, IN _debt DOUBLE, IN _discount DOUBLE, IN _total_pay DOUBLE, IN _account_id INT, IN _note TEXT, IN _old_paid_debt DOUBLE)
BEGIN
